...
export function PendingPayablesWidget({ payables = [], totalUnpaid = 0 }: PendingPayablesWidgetProps) {
  const navigate = useNavigate();
  const displayed = payables.slice(0, 4);
    ...
    <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-lg hover:shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900 dark:hover:shadow-slate-900/50">
      <div className="mb-4 flex items-start justify-between">
        <div className="flex items-start gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-teal-100 to-teal-50 text-teal-600 shadow-sm dark:from-teal-900/30 dark:to-teal-900/10 dark:text-teal-400">
            <Wallet className="h-5 w-5" strokeWidth={1.5} />
          </div>
          <div>
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">Pending Payables</h3>
            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
              {totalUnpaid > 0 ? `${totalUnpaid} unpaid invoice${totalUnpaid !== 1 ? 's' : ''}` : 'All suppliers paid'}
            </p>
          </div>
        </div>
      </div>
          ...
          {displayed.map((inv) => (
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-teal-50 text-teal-500 dark:bg-teal-900/20 dark:text-teal-400">
