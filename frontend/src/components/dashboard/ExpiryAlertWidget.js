...
export function ExpiryAlertWidget() {
    ...
    return (_jsxs("div", { className: "rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-lg dark:border-slate-700 dark:bg-slate-900", children: [_jsxs("div", { className: "flex items-center justify-between mb-4", children: [_jsxs("div", { children: [_jsx("h3", { className: "text-sm font-bold text-slate-900 dark:text-white", children: "Expiry Alerts" }), _jsx("p", { className: "text-[10px] text-slate-400", children: "\u0915\u093E\u0932\u092C\u093E\u0939\u094D\u092F \u0938\u0942\u091A\u0928\u093E" })] }), _jsxs("span", { className: "inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-semibold text-red-600 dark:bg-red-900/20 dark:text-red-400", children: [_jsx(AlertTriangle, { className: "h-3 w-3" }), expired.length + nearWeek.length, " critical"] })] }), _jsx("div", { className: "flex flex-wrap gap-1.5 mb-3", children: categories.map(cat => cat.count > 0 && (_jsxs("span", { className: `inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-medium ${cat.bgColor} ${cat.textColor} ${cat.border} border`, children: [_jsx("span", { className: `h-1.5 w-1.5 rounded-full ${cat.color}` }), cat.label, ": ", cat.count] }, cat.label))) }), _jsx("div", { className: "space-y-1.5 max-h-64 overflow-y-auto pr-1 custom-scrollbar", children: sampleExpiryFull.slice(0, 6).map((item, i) => {
                    ...
                    if (isExpired) {
                        dotColor = 'bg-red-500';
                        rowBg = 'bg-red-50/50 dark:bg-red-950/10';
                        labelText = 'text-red-600 dark:text-red-400';
                        label = 'Expired';
                    }
                    else if (isUrgent) {
                        dotColor = 'bg-orange-500';
                        rowBg = 'bg-orange-50/50 dark:bg-orange-950/10';
                        labelText = 'text-orange-600 dark:text-orange-400';
                        label = `${item.daysLeft}d left`;
                    }
                    else if (isNear) {
                        dotColor = 'bg-amber-500';
                        rowBg = 'bg-amber-50/30 dark:bg-amber-950/5';
                        labelText = 'text-amber-600 dark:text-amber-400';
                        label = `${item.daysLeft}d left`;
                    }
                    return (_jsxs("div", { className: `group flex items-center justify-between rounded-lg ${rowBg} px-3 py-2 transition-all duration-200 hover:shadow-sm`, children: [_jsxs("div", { className: "flex items-center gap-2.5 min-w-0 flex-1", children: [_jsx("span", { className: `h-2 w-2 shrink-0 rounded-full ${dotColor} shadow-sm` }), _jsxs("div", { className: "min-w-0", children: [_jsx("p", { className: "text-[11px] font-medium text-slate-700 dark:text-slate-300 truncate", children: item.name }), _jsxs("p", { className: "text-[9px] text-slate-400", children: [item.batch, " \u00B7 ", formatDate(item.expiryDate)] })] })] }), _jsxs("div", { className: "flex items-center gap-2 shrink-0 ml-2", children: [_jsx("span", { className: `text-[10px] font-semibold ${labelText}`, children: label }), _jsx("span", { className: "text-[10px] font-bold text-slate-800 dark:text-slate-200", children: item.qty })] })] }, i));
                }) })] }));
}
//# sourceMappingURL=ExpiryAlertWidget.js.map