...
export function ExpiryAlertWidget() {
  ...
  const categories = [
    ...
    },
    {
      ...
      count: nearWeek.length,
    },
    {
      label: 'Next 30 Days',
      items: [...nearMonth, ...safe],
      color: 'bg-emerald-500',
      textColor: 'text-emerald-600 dark:text-emerald-400',
      bgColor: 'bg-emerald-50 dark:bg-emerald-950/20',
      border: 'border-emerald-200 dark:border-emerald-900/30',
      count: nearMonth.length + safe.length,
    },
  ];
        ...
        {categories.map(
          (cat) =>
            cat.count > 0 && (
              <span
                key={cat.label}
                className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-medium ${cat.bgColor} ${cat.textColor} ${cat.border} border`}
              >
      ...
      <div className="custom-scrollbar max-h-64 space-y-1.5 overflow-y-auto pr-1">
        {sampleExpiryFull.slice(0, 6).map((item, i) => {
          ...
          return (
            <div
              key={i}
              className={`group flex items-center justify-between rounded-lg ${rowBg} px-3 py-2 transition-all duration-200 hover:shadow-sm`}
            >
              <div className="flex min-w-0 flex-1 items-center gap-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[9px] text-slate-400">
                    {item.batch} · {formatDate(item.expiryDate)}
                  </p>
                </div>
              </div>
