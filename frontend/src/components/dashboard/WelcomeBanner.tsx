...
function getGreeting(): string {
  ...
  const marathiGreeting: Record<string, string> = {
    morning: 'शुभ प्रभात',
    afternoon: 'शुभ दुपार',
    evening: 'शुभ संध्याकाळ',
  };
  if (hour < 12) {return marathiGreeting.morning;}
  if (hour < 17) {return marathiGreeting.afternoon;}
...
export function WelcomeBanner({ userName, companyName = 'Default Company', financialYear = '2025-2026' }: WelcomeBannerProps) {
    ...
    <div className="relative flex h-56 overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-950 shadow-xl shadow-emerald-900/20 sm:h-60">
      ...
      <div className="relative z-10 flex w-full flex-col justify-center px-6 sm:w-3/5 sm:px-8 lg:w-1/2">
        ...
        <p className="mt-1 text-sm leading-relaxed text-emerald-100/70">
          {englishGreeting}! Welcome to your enterprise dashboard.
      ...
      <div className="absolute right-0 top-0 bottom-0 w-32 bg-gradient-to-l from-emerald-950/40 to-transparent sm:block" />
    </div>
  );
}
