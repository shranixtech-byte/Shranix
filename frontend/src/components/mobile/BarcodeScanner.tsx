...
export function BarcodeScanner({ onScan, mode = 'both', continuous = false }: BarcodeScannerProps) {
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
        <div className="text-center">
          ...
          <h3 className="mt-3 text-lg font-semibold text-gray-900 dark:text-white">
            {mode === 'qr' ? 'QR Scanner' : 'Barcode Scanner'}
          </h3>
          <p className="mt-1 text-sm text-gray-500">
            Point your camera at a {mode === 'qr' ? 'QR code' : 'barcode'} to scan
          </p>

          <div className="mt-4 flex flex-col items-center gap-2">
            <button
              onClick={() => startCamera()}
              className="flex items-center gap-2 rounded-xl bg-primary-dark px-6 py-2.5 text-sm font-medium text-white transition-all hover:bg-primary-dark/90"
            >
            ...
            {devices.length > 1 && (
              <select
                onChange={(e) => e.target.value && startCamera(e.target.value)}
                className="mt-2 rounded-lg border border-gray-300 px-3 py-1.5 text-sm dark:border-gray-600 dark:bg-gray-800"
              >
            ...
            <input
              ...
              className="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:border-gray-600 dark:bg-gray-800"
            />
          </div>

          {error && (
            <div className="mt-3 rounded-lg bg-red-50 p-3 text-xs text-red-700 dark:bg-red-900/20 dark:text-red-400">
              {error}
            </div>
          )}
        </div>
  ...
  return (
    <div className="relative overflow-hidden rounded-xl bg-black">
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center justify-center gap-4 bg-gradient-to-t from-black/60 to-transparent p-4">
        ...
        <button
          onClick={stopCamera}
          className="flex items-center gap-2 rounded-full bg-red-500 px-4 py-2 text-sm font-medium text-white shadow-lg"
        >
          <X className="h-4 w-4" />
          Close
        </button>
      ...
      {lastResult && (
        <div className="absolute left-2 right-2 top-2 rounded-lg bg-green-500/90 px-3 py-2 text-sm font-medium text-white">
          ✓ {lastResult.code}
        </div>
      )}
    </div>
...
export function QrCodeGenerator({ value, size = 200, label }: { value: string; size?: number; label?: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      {value && <p className="text-[10px] text-gray-400 break-all max-w-[200px] text-center">{value}</p>}
    </div>
  );
}

export function ScanHistory({ items }: { items: ScanResult[] }) {
  if (items.length === 0) {
    return (
  ...
  return (
    <div className="space-y-2">
      <h3 className="text-sm font-medium text-gray-900 dark:text-white">Scan History</h3>
        ...
        {items.map((item, i) => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-mono text-gray-900 dark:text-white">{item.code}</span>
            </div>
