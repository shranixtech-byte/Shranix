...
type SyncListener = (status: { state: SyncStatus; progress: number; total: number; error?: string }) => void;
...
class SyncEngine {
  ...
  async syncAll(): Promise<SyncStatus> {
    ...
    try {
      ...
      this.notify({ state: 'error', progress: 0, total: 0, error: (error as Error).message });
      return 'error';
    } finally {
  ...
  async syncWithRetry(): Promise<void> {
    ...
    if (status === 'error') {
      ...
      for (let i = 0; i < this.maxRetries; i++) {
        ...
        if (retryStatus === 'completed') {return;}
        delay *= 2;
      }
    }
  }

  async getQueueStats(): Promise<{ pending: number; syncing: number; failed: number; total: number }> {
  ...
  private notify(status: { state: SyncStatus; progress: number; total: number; error?: string }) {
