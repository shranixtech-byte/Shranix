...
type SortKey = 'name' | 'code' | 'status' | 'creditLimit';
...
export function SuppliersPage() {
  ...
  const handleBulkStatus = async () => {
    ...
    if (!window.confirm(`Set ${selected.size} supplier(s) status to "${bulkStatus}"?`)) {
      return;
    }
    setBusy('status');
    try {
      const res = await bulkSupplierStatus([...selected], bulkStatus as any);
      setBulkStatus('');
      setSelected(new Set());
      setError(null);
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !window.confirm(
        `Delete ${selected.size} supplier(s)? Suppliers with purchase documents will be skipped. This cannot be undone.`,
      )
    ) {
      return;
    }
    setBusy('delete');
    try {
      const res = await bulkDeleteSuppliers([...selected]);
      setSelected(new Set());
  ...
  const handleExport = async (format: 'csv' | 'xlsx') => {
    try {
      await downloadSuppliers(format);
    } catch (err) {
      setError((err as Error).message);
    }
  };
  ...
  const handleImport = async (file: File | null) => {
    ...
    try {
      const res = await importSuppliersFile(file, importMode);
      setImportResult(
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between print:hidden">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Supplier Master — search, filter, export & bulk manage · पुरवठादार मास्टर
          </p>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/80 text-xs uppercase tracking-wide text-slate-500 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-400">
                ...
                {(
                  [
                    { key: 'code' as SortKey, label: 'Code', className: 'w-28' },
                    { key: 'name' as SortKey, label: 'Supplier', className: 'min-w-[200px]' },
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={10} className="px-4 py-16 text-center">
                    ...
                    <p className="mt-2 text-sm font-medium text-slate-500">No suppliers found</p>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        <IconBtn title="View" onClick={() => navigate(`/suppliers/${s.id}`)}>
                          <Eye className="h-3.5 w-3.5" />
                        </IconBtn>
                        <IconBtn title="Edit" onClick={() => navigate(`/suppliers/${s.id}/edit`)}>
                          <Pencil className="h-3.5 w-3.5" />
                        </IconBtn>
                        <IconBtn
                          title="Ledger"
                          onClick={() => navigate(`/suppliers/${s.id}?tab=ledger`)}
                        >
