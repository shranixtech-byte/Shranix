// ═════════════════════════════════════════════════════════
// SALES ORDER SHARE MODAL (Print · Email · WhatsApp · PDF)
// ═════════════════════════════════════════════════════════
...
} from '@/services/sales-order-pdf.service';
...
@media print {
  ...
  #order-pdf-preview, #order-pdf-preview * { visibility: visible; }
  #order-pdf-preview {
...
const safeName = (n: string) => String(n || 'order').replace(/[^A-Za-z0-9._-]/g, '_');
...
export function SalesOrderShareModal({
  orderId,
  onClose,
}: {
  orderId: string;
  ...
  const [data, setData] = useState<SalesOrderPdfData | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data: d, orderNumber: num } = await loadSalesOrderPdfData(orderId);
        if (cancelled) {
          return;
        }
        setData(d);
        ...
        setEmailSubject(`Sales Order ${num} from ${d.companyName || 'Company'}`);
        setEmailMessage(
          `Namaste ${d.customerName},\n\nAapka sales order ${num} is email ke saath hai.\nTotal: ${fmtINR(d.grandTotal)}\n\nDhanyawad!\n${d.companyName || 'Company'}`,
        );
        setWaMobile(d.customerMobile || '');
        setWaMessage(
          `Namaste ${d.customerName},\n\nAapka sales order ${num} taiyar hai.\nTotal: ${fmtINR(d.grandTotal)}\n\nDhanyawad!\n${d.companyName || 'Company'}`,
        );
      } catch (err) {
        if (!cancelled) {
  ...
  const previewHtml = useMemo(() => (data ? renderSalesOrderPdf(data) : ''), [data]);
  ...
  const handleDownload = useCallback(async () => {
    ...
    setActionError(null);
    try {
      await downloadSalesOrderPdf(orderId);
    } catch (err) {
  ...
  const handleSendEmail = useCallback(async () => {
    ...
    try {
      ...
      const name = safeName(orderNumber);
  ...
  }, [emailTo, emailSubject, emailMessage, orderId, orderNumber]);
  ...
  }, [waMobile, waMessage, orderId, orderNumber]);
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-slate-200/70 p-6 dark:bg-slate-900/50">
          <div
            ...
            className="mx-auto w-fit origin-top shadow-2xl transition-all"
