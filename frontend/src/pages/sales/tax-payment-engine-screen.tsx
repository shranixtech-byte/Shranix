...
const INPUT_CLS =
  'h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-xs outline-none focus:border-emerald-500 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100';
const INPUT_CLS_R =
  'h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-right text-xs outline-none focus:border-emerald-500 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100';
...
function InfoCell({
  ...
  return (
    <div>
      ...
      <p className={cn('text-sm font-medium text-slate-900 dark:text-slate-100', className)}>
        {value}
      </p>
    </div>
  );
...
interface GstEngineProps {
  ...
  onCessTypeChange: (t: CessType) => void;
  onCessValueChange: (v: number) => void;
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
        <div className="flex items-center gap-2 border-b border-slate-100 px-4 py-3 dark:border-slate-700">
          ...
          {isInterState ? (
            <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-purple-100 px-2 py-0.5 text-[10px] font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">
              Interstate · IGST
            </span>
          ) : (
            <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
        ...
        <div className="space-y-4 p-4">
          <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
            {GST_CATEGORIES.map((cat) => (
              <button
                key={cat.value}
                type="button"
                onClick={() => onGstCategoryChange(cat.value)}
                className={cn(
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  maxLength={15}
                  className={cn(
                    'h-10 w-full rounded-lg border bg-white px-3 font-mono text-sm outline-none transition-all focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100',
                    customerGstin &&
                      gstinValid &&
                      'border-emerald-400 bg-emerald-50/30 dark:border-emerald-600',
                    customerGstin &&
                      !gstinValid &&
              ...
              {customerGstin && (
                <span
                  className={cn(
                    'text-xs font-medium',
                    gstinValid ? 'text-emerald-600' : 'text-red-500',
                  )}
                >
            ...
            {customerGstin.length >= 2 && (
              <p className="mt-1 text-[10px] text-slate-400">
                State: {stateCode} - {stateName} {isInterState ? '(→ IGST)' : '(→ CGST+SGST)'}
              </p>
            )}
          </div>
          ...
          <div className="border-t border-slate-100 pt-4 dark:border-slate-700">
            ...
            <div className="flex items-center gap-3">
              <select
                value={cessType}
                onChange={(e) => onCessTypeChange(e.target.value as CessType)}
              ...
              <div className="relative max-w-[120px] flex-1">
                <input
                  type="number"
                  value={cessValue || ''}
                  onChange={(e) => onCessValueChange(Math.max(0, parseFloat(e.target.value) || 0))}
                  min={0}
                  step={0.01}
                  className="h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-right text-xs outline-none focus:border-emerald-500 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                  aria-label="CESS value"
                />
                <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-slate-400">
                  {cessType === 'percentage' ? '%' : cessType === 'flat' ? '₹' : '₹/qty'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg bg-slate-50 px-3 py-2 dark:bg-slate-700/30">
            ...
            <div className="flex flex-wrap gap-2 text-[11px] text-slate-600 dark:text-slate-400">
              {isInterState ? (
                <span>IGST: Full rate applicable</span>
              ) : (
                <>
                  <span>CGST: Half rate</span>
                  <span>SGST: Half rate</span>
                </>
              )}
              {gstCategory === 'composition' && (
                <span className="text-amber-600">Composition dealer — limited ITC</span>
              )}
              {gstCategory === 'zero_rated' && (
                <span className="text-blue-600">0% GST — Refund eligible</span>
              )}
              {gstCategory === 'export' && (
                <span className="text-purple-600">Export — LUT required</span>
              )}
            </div>
          </div>
        </div>
...
const PaymentDetailsForm = memo(function PaymentDetailsForm({
  split,
  onUpdate,
  onRemove,
  ...
  return (
    <div className="rounded-lg border border-slate-100 bg-slate-50/50 p-3 dark:border-slate-700 dark:bg-slate-800/30">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex h-6 w-6 items-center justify-center rounded text-slate-400 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20"
          >
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          <FormField label="Cheque No">
            <input
              ...
              onChange={(e) => onUpdate(split.id, 'chequeNumber', e.target.value)}
              className={INPUT_CLS}
            />
          </FormField>
          <FormField label="Bank">
            <input
              ...
              onChange={(e) => onUpdate(split.id, 'bankName', e.target.value)}
              className={INPUT_CLS}
            />
          </FormField>
          <FormField label="Cheque Date">
            <input
              type="date"
              value={split.chequeDate}
              onChange={(e) => onUpdate(split.id, 'chequeDate', e.target.value)}
              className={INPUT_CLS}
            />
          </FormField>
          <FormField label="Branch">
            <input
              type="text"
              value={split.branch}
              onChange={(e) => onUpdate(split.id, 'branch', e.target.value)}
              className={INPUT_CLS}
            />
          </FormField>
        </div>
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          <FormField label="Transaction ID">
            <input
              type="text"
              value={split.refNo}
              onChange={(e) => onUpdate(split.id, 'refNo', e.target.value)}
              className={INPUT_CLS}
            />
          </FormField>
          ...
          <FormField label="Transfer Date">
            <input
              type="date"
              value={split.transactionDate}
              onChange={(e) => onUpdate(split.id, 'transactionDate', e.target.value)}
              className={INPUT_CLS}
            />
          </FormField>
        </div>
      ...
      {split.method === 'upi' && (
        <div className="mt-2 grid grid-cols-2 gap-2">
          <FormField label="UPI Ref No">
            <input
              type="text"
              value={split.upiRef}
              onChange={(e) => onUpdate(split.id, 'upiRef', e.target.value)}
              className={INPUT_CLS}
            />
          </FormField>
          <FormField label="App">
            <input
              type="text"
              value={split.upiApp}
              onChange={(e) => onUpdate(split.id, 'upiApp', e.target.value)}
      ...
      {split.method === 'credit' && (
        <div className="mt-2 grid grid-cols-2 gap-2">
          <FormField label="Credit Days">
            <input
              type="number"
              value={split.creditDays || ''}
              onChange={(e) => onUpdate(split.id, 'creditDays', parseInt(e.target.value) || 0)}
              className={INPUT_CLS}
            />
          </FormField>
          <FormField label="Due Date">
            <input
              type="date"
              value={split.dueDate}
              onChange={(e) => onUpdate(split.id, 'dueDate', e.target.value)}
              className={INPUT_CLS}
            />
          </FormField>
          <FormField label="Outstanding">
            <input
              type="number"
              value={split.outstanding || ''}
              onChange={(e) => onUpdate(split.id, 'outstanding', parseFloat(e.target.value) || 0)}
              className={INPUT_CLS_R}
            />
          </FormField>
          <FormField label="Credit Limit">
            <input
              type="number"
              value={split.creditLimit || ''}
              onChange={(e) => onUpdate(split.id, 'creditLimit', parseFloat(e.target.value) || 0)}
              className={INPUT_CLS_R}
            />
          </FormField>
          {split.outstanding > split.creditLimit && split.creditLimit > 0 && (
            <div className="col-span-full flex items-center gap-1.5 rounded bg-red-50 px-2 py-1 text-[10px] font-medium text-red-600 dark:bg-red-900/10 dark:text-red-400">
              <Info className="h-3 w-3" /> Credit limit exceeded by{' '}
              {formatINR(split.outstanding - split.creditLimit)}
            </div>
          )}
        </div>
      )}
      ...
      {split.method === 'card' && (
        <div className="mt-2 grid grid-cols-2 gap-2">
          <FormField label="Card Type">
            <select
              value={split.cardType}
              onChange={(e) => onUpdate(split.id, 'cardType', e.target.value)}
              className={INPUT_CLS}
            >
    ...
    <div className="rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
      <div className="flex items-center gap-2 border-b border-slate-100 px-4 py-3 dark:border-slate-700">
        ...
        <span className="ml-auto text-xs text-slate-400">
          {formatINR(totalPaid)} / {formatINR(grandTotal)}
        </span>
      </div>
          ...
          {PAYMENT_METHODS.map((method) => {
            ...
            return (
              <button
                key={method.value}
                type="button"
                onClick={() => onAddSplit(method.value)}
                disabled={isActive}
                className={cn(
              ...
              >
                ...
                <span className="text-[9px] font-semibold">{method.label}</span>
              </button>
            );
          })}
        </div>
              ...
              <div key={split.id}>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    value={split.amount || ''}
                    onChange={(e) =>
                      onUpdateSplit(split.id, 'amount', parseFloat(e.target.value) || 0)
                    }
                    min={0}
                    step={0.01}
        ...
        >
          <span className="text-xs font-medium">
            {remaining === 0
              ? '✓ Fully Paid'
              : overPayment
                ? `⚠ Over by ${formatINR(totalPaid - grandTotal)}`
                : 'Remaining'}
          </span>
          <span className="text-sm font-bold">
            {formatINR(Math.max(0, grandTotal - totalPaid))}
          </span>
        </div>
      </div>
    </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-100 px-5 py-2.5 dark:border-slate-700">
        ...
        <span
          className={cn(
            ...
            isInterState ? 'text-purple-600' : 'text-blue-600',
          )}
        >
          {isInterState ? 'IGST' : 'CGST+SGST'}
        </span>
      </div>
      <div className="grid grid-cols-2 gap-4 px-5 py-3 sm:grid-cols-3 lg:grid-cols-7">
        ...
        {isInterState ? (
          ...
          <>
            <Cell label="CGST" value={formatINR(cgst)} className="text-blue-600" />
            <Cell label="SGST" value={formatINR(sgst)} className="text-blue-600" />
          </>
        )}
...
export function TaxPaymentEngineScreen({
  ...
  const isInterState = useMemo(() => {
    if (gstCategory === 'interstate' || gstCategory === 'export') {
      return true;
    }
  ...
  const gstResult = useMemo(() => {
    ...
    if (cessType === 'percentage') {
      cess = Math.round(taxable * (cessValue / 100) * 100) / 100;
    } else if (cessType === 'flat') {
      cess = cessValue;
  ...
  useEffect(() => {
    ...
    if (splits.length > 0 && totalPaid < gstResult.grandTotal * 0.99) {
      errors.push(`Shortfall: ${formatINR(gstResult.grandTotal - totalPaid)} remaining`);
    }
    if (overPayment) {
      errors.push(`Overpayment by ${formatINR(totalPaid - gstResult.grandTotal)}`);
    }
    if (gstCategory === 'reverse_charge') {
      errors.push('Reverse charge — RCM accounting required');
    }
  ...
  const handleAddSplit = useCallback(
    (method: PaymentMethod) => {
      setSplits((prev) => {
        if (prev.some((s) => s.method === method)) {
          return prev;
        }
  ...
  return (
    <div
      className="animate-in fade-in flex h-full flex-col duration-200"
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Configure GST, taxes, and payment details
            </p>
          </div>
        </div>
      </div>
      ...
      {validationErrors.length > 0 && (
        <div className="space-y-1 px-6 pt-3">
          {validationErrors.map((err, i) => (
            <div
              key={i}
              className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700 dark:border-amber-800 dark:bg-amber-900/10 dark:text-amber-400"
            >
      ...
      <div className="min-h-0 flex-1 overflow-auto px-6 py-4">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div
            ...
            className={cn('rounded-xl', focusMode === 'left' && 'ring-2 ring-emerald-500/20')}
          >
            <GstEngine
              gstCategory={gstCategory}
              cessType={cessType}
              ...
              onGstCategoryChange={setGstCategory}
              onCessTypeChange={setCessType}
              onCessValueChange={setCessValue}
          ...
          <div
            ...
            className={cn('rounded-xl', focusMode === 'right' && 'ring-2 ring-emerald-500/20')}
          >
            <PaymentEngine
              splits={splits}
              grandTotal={gstResult.grandTotal}
      ...
      <div className="px-6 pb-3">
        <FinalSummary
          ...
          taxableAmount={gstResult.taxable}
          cgst={gstResult.cgst}
          sgst={gstResult.sgst}
          igst={gstResult.igst}
          cess={gstResult.cess}
