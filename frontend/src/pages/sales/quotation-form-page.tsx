...
// ═════════════════════════════════════════════════════════
// QUOTATION CREATE PAGE — STEP BY STEP BUILD
// (Invoice page jaisa hi single-box design — Quote No, Date,
//  Valid Till, Customer, Items, Summary aur sab kuch EK BOX mein)
// ═════════════════════════════════════════════════════════

const DELIVERY_OPTIONS = [
  { label: 'Immediate', value: 'immediate' },
  { label: 'Within 2 Days', value: '2_days' },
...
// Keyboard key chip — shortcut hints (F2, F3, F5, F6, Esc)
...
// Shortcut list — left rail + mobile strip dono mein reuse
...
export function SalesQuotationFormPage() {
  ...
  const quoteDatePickerRef = useRef<HTMLInputElement>(null);
  const validTillPickerRef = useRef<HTMLInputElement>(null);
  ...
  const [lastSaveMode, setLastSaveMode] = useState<'draft' | 'submit'>('draft');
  const [quoteId, setQuoteId] = useState(id || '');
  const [shareOpen, setShareOpen] = useState(false);

  // ── Load branches + existing quotation (edit mode) ──
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await apiRequest<any>('/branches?pageSize=200');
        const body =
      ...
      if (isEditing && id) {
        try {
          const q = await apiRequest<any>(`/sales/quotations/${id}`);
          const rec = q?.data ?? q ?? {};
          ...
          if (!cancelled) {
            setExistingStatus(String(rec.status || ''));
            setQuoteId(id);
            setQuoteNumber(rec.quoteNumber || '');
            setQuoteDate(String(rec.quoteDate || todayISO()).slice(0, 10));
            setValidTill(String(rec.validTill || '').slice(0, 10));
            ...
            setDeliveryTime(
              DELIVERY_OPTIONS.some((d) => d.value === rec.deliveryTime)
                ? rec.deliveryTime || ''
                : rec.deliveryTime
                  ? 'custom'
                  : '',
            ...
            setFreight(String(Number(rec.freight) || 0));
            setInstallationCharges(String(Number(rec.installationCharges) || 0));
            setWarranty(rec.warranty || '');
  ...
  // Product + qty/rate/discount → InvoiceLineItem
  ...
  const addPendingItem = () => {
    ...
    const line = buildQuoteLine(pendingProduct, qty, rate, discPct, discAmt);
    // Same product + same batch → qty merge (nayi line nahi)
    setItems((prev) => {
  ...
  const handleScannedProduct = (product: ProductRecord) => {
    ...
    const firstBatch = product.batches?.find((b) => b.availableQty > 0) ?? product.batches?.[0];
    const line = buildQuoteLine(product, qty, rate, 0, 0);
    setItems((prev) => {
      const existing = prev.find(
  ...
  // ── Totals (invoice jaisa hi chain: Basic → Disc → Taxable → GST → Freight → Round) ──
  ...
  // Summary — freight + installation + round off + grand total
  const summary = useMemo(() => {
    const freightVal = Math.max(0, parseFloat(freight) || 0);
    const installationVal = Math.max(0, parseFloat(installationCharges) || 0);
    const netBillTotal = totals.grandTotal + freightVal + installationVal;
    ...
    return { freightVal, installationVal, netBillTotal, netBillAmt, roundOff, finalAmt };
  }, [totals.grandTotal, freight, installationCharges]);
  ...
  // ── Save: quotation ko backend mein save karo ──────────────────────────
  // mode 'draft'  = sirf draft save
  // mode 'submit' = draft save + approval ke liye submit (approval chain)
  // mode 'print'  = draft save + print/share modal kholo
  const canSubmit = !isEditing || !existingStatus || existingStatus === 'draft';

  const saveQuote = async (mode: 'draft' | 'submit' | 'print'): Promise<string | null> => {
    setSaveError(null);
    setSaveSuccess(false);
    setHasSaved(false);
    ...
    if (!quoteDate) {
      setSaveError('Quote date required hai');
      return null;
    }
    setSaving(true);
    ...
    try {
      const effectiveDelivery =
        deliveryTime === 'custom' ? (customDeliveryTime || '').trim() || 'custom' : deliveryTime;
      const payload = {
        ...
        deliveryTime: effectiveDelivery || undefined,
        freight: summary.freightVal,
        installationCharges: summary.installationVal,
        warranty: warranty || undefined,
        customerNotes: customerNotes || undefined,
        notes: notes || undefined,
        terms: terms || undefined,
        basicTotal: totals.itemTotal,
      ...
      const res =
        isEditing && id
          ? await apiRequest(`/sales/quotations/${id}`, { method: 'PUT', body })
          : await apiRequest('/sales/quotations', { method: 'POST', body });
      const rec = (res as { data?: any })?.data ?? res ?? {};
      const newId = id || String(rec?.id || '');
      const savedNumber = String(rec?.quoteNumber || quoteNumber || '');

      if (mode === 'submit' && canSubmit) {
        await apiRequest(`/sales/quotations/${newId}/submit-approval`, {
          method: 'POST',
          body: JSON.stringify({}),
        });
      }
      ...
      setQuoteId(newId);
      setExistingStatus(mode === 'submit' ? 'pending' : existingStatus);
      setQuoteNumber(savedNumber);
      setLastSavedNumber(savedNumber);
      setLastSaveMode(mode === 'submit' ? 'submit' : 'draft');
      setSaveSuccess(true);
      setHasSaved(true);
      if (mode === 'print') {
        setShareOpen(true);
      ...
      setSaveError((err as Error).message || 'Quotation save nahi hua — dobara try karo');
      return null;
    } finally {
      setSaving(false);
  ...
  const handleSaveSubmit = (): Promise<string | null> => saveQuote('submit');
  const handleSaveDraft = (): Promise<string | null> => saveQuote('draft');

  // Save & Print — saveQuote('print') khud hi share/print modal kholta hai jab save successful ho
  const handlePrint = async () => {
    ...
    await saveQuote('print');
  };

  // Esc / keyboard shortcuts — mouse ke bina pura flow
  const kbRef = useRef({
    openCustomerSearch,
    openItemSearch,
    handleSaveSubmit,
  ...
  kbRef.current = { openCustomerSearch, openItemSearch, handleSaveSubmit, handlePrint, navigate };
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        kbRef.current.navigate('/sales/quotations');
        return;
      }
      switch (e.key) {
        case 'F2':
        ...
        case 'F5':
          e.preventDefault();
          if (!saving) {
            void kbRef.current.handleSaveSubmit();
          }
          break;
        case 'F6':
          e.preventDefault();
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-3 text-slate-400">
          ...
          <p className="text-sm">Quotation load ho raha hai...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-in fade-in duration-300 lg:flex lg:items-start lg:gap-5">
        ...
        <div className="mx-auto w-full max-w-6xl px-4 lg:px-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
                {isEditing ? 'Edit Sales Quotation' : 'Create Sales Quotation'}
              </h1>
              <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                Sab kuch ek hi box mein — items, pricing aur approval ek saath
              </p>
            </div>
          </div>

          {/* Ek hi box — Quote No + Date + Valid Till (aage items bhi yahin) */}
          <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800/50">
            <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
              {/* Quote No — auto (server side numbering) ya manual */}
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  placeholder="AUTO"
                  title="Auto numbering ON hai to save par khud generate hoga — manual ke liye yahan type karo"
                ...
                <span className="text-[11px] text-slate-400">
                  FY {quoteDate ? shortFinancialYear(new Date(quoteDate)) : ''}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 border-l border-slate-200 pl-6 dark:border-slate-700">
                ...
                <button
                  ...
                  onClick={() => {
                    try {
                      quoteDatePickerRef.current?.showPicker();
                    } catch {
                      quoteDatePickerRef.current?.focus();
                    }
                  }}
                  ...
                  title="Quote date change karo"
                >
                  {formatDateDDMMYYYY(quoteDate)}
              ...
              <div className="flex items-center gap-2 border-l border-slate-200 pl-6 dark:border-slate-700">
                ...
                <button
                  ...
                  onClick={() => {
                    try {
                      validTillPickerRef.current?.showPicker();
                    } catch {
                      validTillPickerRef.current?.focus();
                    }
                  }}
                  className={cn(
                  ...
                  title="Quotation kahan tak valid hai (khali chhodo to settings se auto-fill)"
                >
                  {validTill ? formatDateDDMMYYYY(validTill) : 'Auto / Select'}
                  <ChevronDown className="h-3.5 w-3.5 text-slate-400" />
                </button>
              </div>
            </div>
            ...
            {/* Customer saved details — mobile/GST/email/city chips */}
            ...
            {/* Quote Options — collapsible (quotation-specific fields) */}
            <div className="mt-3">
              <button
                type="button"
              ...
              >
                ...
                Quote Options (Payment · Delivery · Shipping · Notes)
                <ChevronDown
                ...
                <div className="mt-3 grid gap-4 rounded-xl border border-slate-200 bg-slate-50/60 p-4 sm:grid-cols-2 dark:border-slate-700 dark:bg-slate-900/40">
                  ...
                  {deliveryTime === 'custom' && (
                    <FormInput
                      ...
                      placeholder="e.g. After monsoon / on order confirmation"
                  ...
                  <FormTextarea
                    ...
                    placeholder="Visible to customer (printed on quotation)"
                    rows={2}
                    value={customerNotes}
            ...
            <div className="mt-4">
              ...
              <div className="mt-3 flex flex-col gap-3 lg:flex-row">
                ...
                <div className="w-full shrink-0 space-y-2.5 rounded-lg border border-slate-200 bg-slate-50/60 p-5 lg:w-80 dark:border-slate-700 dark:bg-slate-800/40">
                  ...
                  {/* Installation Charges — editable */}
                  <label className="flex items-center justify-between gap-2 text-sm">
                    <span className="text-slate-600 dark:text-slate-300">Installation Charges</span>
                    <div className="relative">
                      <span className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-xs text-slate-400">
                        ₹
                      </span>
                      <input
                        ...
                        placeholder="0"
                        className="h-8 w-24 rounded-md border border-slate-200 bg-white pl-5 pr-2 text-right text-sm font-medium text-slate-800 outline-none transition-colors focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200"
                        aria-label="Installation charges"
                      />
                    </div>
                  </label>
                  ...
                  {hasSaved && (
                    ...
                    >
                      <SummaryRow
                        label={
                          lastSaveMode === 'submit' ? 'Status: Pending Approval' : 'Status: Draft'
                        }
            ...
            {saveSuccess && (
              ...
              >
                {lastSaveMode === 'submit'
                  ? `✅ Quotation save + approval ke liye submit ho gayi! (${lastSavedNumber})`
                  : `📝 Quotation save ho gayi! (${lastSavedNumber})`}
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <Button
                ...
                title="Quotation cancel karke list par jao (Esc)"
              >
                <X className="h-4 w-4" />
                Cancel
                <Kbd>Esc</Kbd>
              ...
              <Button
                ...
                title="Draft save karo (approval submit nahi hota)"
              >
                <FileText className="h-4 w-4" />
                Save Draft
              </Button>

              {/* Save & Submit — draft + approval chain (F5) */}
              <Button
                variant="secondary"
                onClick={() => void handleSaveSubmit()}
                disabled={saving || (isEditing && !canSubmit)}
                className="flex min-w-[150px] items-center justify-center gap-1.5 border-emerald-200 bg-emerald-50 text-emerald-700 hover:border-emerald-300 hover:bg-emerald-100 hover:text-emerald-800 disabled:cursor-not-allowed disabled:opacity-40 dark:border-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300 dark:hover:border-emerald-700 dark:hover:bg-emerald-900/50 dark:hover:text-emerald-200"
                title={
                  canSubmit
                    ? 'Quotation save + approval chain (Executive → Manager → Owner) mein submit karo (F5)'
                    : 'Ye quotation pehle hi submit ho chuki hai'
                }
              >
                <Check className="h-4 w-4" />
                {isEditing && !canSubmit ? 'Save' : 'Save & Submit'}
                {canSubmit && <Kbd>F5</Kbd>}
              </Button>

              {/* Save & Print — save + PDF/share modal (F6) */}
              <Button
                variant="secondary"
                ...
                title="Quotation save + print/PDF/WhatsApp/Email share modal kholo (F6)"
              >
              ...
              {/* PDF & Share — save hone ke baad hi (Email/WhatsApp/Print/Duplicate modal) */}
              <Button
                variant="secondary"
                onClick={() => {
                  if (quoteId) {
                ...
                title={
                  quoteId
                    ? 'Print · Download PDF · Email PDF · WhatsApp PDF · Duplicate Copy'
                    : 'Pehle quotation save karo — phir PDF/WhatsApp/Email bhej sakte ho'
                }
              >
      ...
      {/* Share / PDF modal — print preview + email + whatsapp + duplicate copy */}
      {shareOpen && quoteId && (
        <QuotationShareModal quoteId={quoteId} onClose={() => setShareOpen(false)} />
      )}
