  ...
  useEffect(() => {
    if (isEditing && id) {
      ...
      apiRequest<SupplierForm>(`/suppliers/${id}`)
        .then((data) => { if (data && typeof data === 'object') setForm({ ...initialForm, ...data } as SupplierForm); })
  ...
  const sections: FormSection[] = [
    {
      ...
      fields: (
        <>
          <FormInput label="Supplier Name" required value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="Enter supplier name" />
          <FormInput label="Supplier Code" value={form.code} onChange={(e) => update('code', e.target.value)} placeholder="SUP-001" />
          ...
          <FormInput label="Email" type="email" value={form.email} onChange={(e) => update('email', e.target.value)} placeholder="supplier@example.com" />
        </>
      ),
    },
    {
      ...
      fields: (
        <>
          <div className="grid gap-4 sm:grid-cols-2">
            <FormInput label="Bank Name" value={form.bankName} onChange={(e) => update('bankName', e.target.value)} placeholder="Bank name" />
            <FormInput label="Account Number" value={form.bankAccountNo} onChange={(e) => update('bankAccountNo', e.target.value)} placeholder="Account number" />
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <FormInput label="IFSC Code" value={form.bankIfsc} onChange={(e) => update('bankIfsc', e.target.value)} placeholder="IFSC code" />
            <FormInput label="Bank Branch" value={form.bankBranch} onChange={(e) => update('bankBranch', e.target.value)} placeholder="Branch name" />
          </div>
        </>
      ),
    },
