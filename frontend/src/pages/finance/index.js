...
const groupColumns = [
    ...
    { key: 'type', label: 'Type', render: (v) => {
            const types = { assets: '📊 Assets', liabilities: '📋 Liabilities', income: '📈 Income', expenses: '📉 Expenses', equity: '🏛️ Equity' };
...
export function AccountGroupsPage() {
    return (_jsx(MasterDataPage, { title: "Account Groups", description: "Nested hierarchy of account groups: Assets, Liabilities, Income, Expenses, Equity", columns: groupColumns, apiPath: "/finance/account-groups", formFields: groupFields }));
}
// ═════════════════════════════════════════════════════════
// 2. CHART OF ACCOUNTS
// ═════════════════════════════════════════════════════════
const coaColumns = [
    ...
    { key: 'accountType', label: 'Type', render: (v) => {
            const types = { assets: 'Assets', liabilities: 'Liabilities', income: 'Income', expenses: 'Expenses', equity: 'Equity' };
            return types[v] || String(v);
        } },
...
export function ChartOfAccountsPage() {
    return (_jsx(MasterDataPage, { title: "Chart of Accounts", description: "Complete chart of accounts with auto account codes, opening balances, and GST/bank settings", columns: coaColumns, apiPath: "/finance/chart-of-accounts", formFields: coaFields }));
}
// ═════════════════════════════════════════════════════════
// 3. LEDGER MASTER
// ═════════════════════════════════════════════════════════
const ledgerColumns = [
    ...
    { key: 'ledgerType', label: 'Type', render: (v) => {
            const types = { customer: '👤 Customer', supplier: '🏭 Supplier', cash: '💵 Cash', bank: '🏦 Bank', expense: '📉 Expense', income: '📈 Income', tax: '🧾 Tax' };
            return types[v] || String(v);
        } },
...
export function LedgerMasterPage() {
    return (_jsx(MasterDataPage, { title: "Ledger Master", description: "Customer, supplier, cash, bank, expense, income, and tax ledgers with balance tracking", columns: ledgerColumns, apiPath: "/finance/ledgers", formFields: ledgerFields }));
}
// ═════════════════════════════════════════════════════════
// 4. JOURNAL ENTRIES
// ═════════════════════════════════════════════════════════
...
export function JournalEntriesPage() {
    return (_jsx(MasterDataPage, { title: "Journal Entries", description: "Double-entry vouchers with debit/credit validation, narration, attachments, and approval workflow", columns: journalColumns, apiPath: "/finance/journal-entries", formFields: journalFields }));
}
// ═════════════════════════════════════════════════════════
// 5. CASH BOOK
// ═════════════════════════════════════════════════════════
const cashBookColumns = [
    ...
    { key: 'voucherType', label: 'Type', render: (v) => v === 'receipt' ? '📥 Receipt' : '📤 Payment' },
...
export function CashBookPage() {
    return (_jsx(MasterDataPage, { title: "Cash Book", description: "Cash receipts and payments with running balance, opening/closing balance tracking", columns: cashBookColumns, apiPath: "/finance/cash-book", formFields: cashBookFields }));
}
// ═════════════════════════════════════════════════════════
// 6. BANK BOOK
// ═════════════════════════════════════════════════════════
const bankBookColumns = [
    { key: 'voucherNumber', label: 'Voucher#' },
    { key: 'entryDate', label: 'Date' },
    { key: 'voucherType', label: 'Type' },
    ...
    { key: 'credit', label: 'Credit ₹', render: (v) => `₹${Number(v || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}` },
    { key: 'runningBalance', label: 'Balance ₹', render: (v) => `₹${Number(v || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}` },
    { key: 'reconciliationStatus', label: 'Recon', render: (v) => getStatusBadge(v) },
];
const bankBookFields = [
...
export function BankBookPage() {
    return (_jsx(MasterDataPage, { title: "Bank Book", description: "Bank receipts, payments, and transfers with cheque/UTR tracking and reconciliation", columns: bankBookColumns, apiPath: "/finance/bank-book", formFields: bankBookFields }));
}
// ═════════════════════════════════════════════════════════
// 7. COST CENTERS
// ═════════════════════════════════════════════════════════
const costCenterColumns = [
    ...
    { key: 'name', label: 'Name' },
    { key: 'type', label: 'Type', render: (v) => {
            const types = { department: '🏢 Dept', project: '📋 Project', branch: '🏪 Branch', warehouse: '🏭 Warehouse', profit_center: '💰 Profit' };
            return types[v] || String(v);
        } },
...
export function CostCentersPage() {
    return (_jsx(MasterDataPage, { title: "Cost Centers", description: "Hierarchical cost centers for departmental, project, and branch-wise accounting", columns: costCenterColumns, apiPath: "/finance/cost-centers", formFields: costCenterFields }));
}
// ═════════════════════════════════════════════════════════
...
export function AccountingSettingsPage() {
    return (_jsx(MasterDataPage, { title: "Accounting Settings", description: "Global accounting configuration: voucher numbering, approval, default accounts, currency", columns: [{ key: 'autoVoucherNumber', label: 'Auto Voucher#' }, { key: 'currency', label: 'Currency' }, { key: 'requireApproval', label: 'Approval Required', render: (v) => v ? '✅ Yes' : '❌ No' }], apiPath: "/finance/settings", formFields: settingsFields }));
}
