...
// ── Near Expiry ─────────────────────────────────────────
const nearExpiryColumns: ColumnDef[] = [
  ...
  { key: 'daysRemaining', label: 'Days Left', render: (v) => {
    const d = Number(v);
    return <span className={d <= 15 ? 'text-red-600 font-semibold' : d <= 30 ? 'text-amber-600 font-semibold' : 'text-emerald-600'}>{d}d</span>;
  }},
  { key: 'warehouseId', label: 'Warehouse' },
];

const nearExpiryFields: FormField[] = [
...
export function NearExpiryPage() {
  return (
    <MasterDataPage
      ...
      description="Track products approaching their expiry date for timely action"
      columns={nearExpiryColumns}
      apiPath="/inventory/batches"
      formFields={nearExpiryFields}
    />
  );
}

// ── Damage Register ──────────────────────────────────────
const damageColumns: ColumnDef[] = [
...
export function DamageRegisterPage() {
  return (
    <MasterDataPage
      ...
      description="Record and track damaged inventory for insurance claims and write-offs"
      columns={damageColumns}
      apiPath="/inventory/damage-register"
      formFields={damageFields}
    />
  );
}

// ── Recall Register ──────────────────────────────────────
const recallColumns: ColumnDef[] = [
  { key: 'createdAt', label: 'Date', render: (v) => v ? new Date(v as string).toLocaleDateString('en-IN') : '—' },
  { key: 'itemId', label: 'Item' },
  { key: 'batchNo', label: 'Batch' },
  { key: 'quantity', label: 'Qty' },
...
export function RecallRegisterPage() {
  return (
    <MasterDataPage
      ...
      description="Manage product recalls for quality, safety, and compliance"
      columns={recallColumns}
      apiPath="/inventory/recall-register"
      formFields={recallFields}
    />
  );
}
...
const returnColumns: ColumnDef[] = [
  ...
  { key: 'expectedDate', label: 'Expected', render: (v) => v ? new Date(v as string).toLocaleDateString('en-IN') : '—' },
...
export function DistributorReturnsPage() {
  return (
    <MasterDataPage
      title="Distributor Return Queue"
      description="Track and manage returns from distributors with replacement and resolution workflow"
      columns={returnColumns}
      apiPath="/inventory/distributor-returns"
...
// ── Replacement Queue ────────────────────────────────────
const replacementColumns: ColumnDef[] = [
  { key: 'createdAt', label: 'Date', render: (v) => v ? new Date(v as string).toLocaleDateString('en-IN') : '—' },
  { key: 'itemId', label: 'Item' },
  { key: 'batchNo', label: 'Batch' },
  { key: 'quantity', label: 'Qty' },
...
const replacementFields: FormField[] = [
  { name: 'itemId', label: 'Item ID', type: 'text', required: true },
  { name: 'batchNo', label: 'Batch No', type: 'text' },
...
export function ReplacementQueuePage() {
  return (
    <MasterDataPage
      ...
      description="Track product replacements issued to distributors for returned or damaged goods"
      columns={replacementColumns}
      apiPath="/inventory/replacement-queue"
      formFields={replacementFields}
    />
  );
}
