          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50 dark:border-slate-700 dark:bg-slate-800/80">
                <th className="sticky top-0 px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                  #
                </th>
                {columns.map((col) => (
                  <th
                    key={col.key}
                    className="sticky top-0 px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500"
                  >
                    {col.label}
                  </th>
                ))}
                <th className="sticky top-0 px-4 py-3.5 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">
                  Actions
                </th>
              </tr>
            </thead>
                ...
                state.data.map((record, idx) => (
                  ...
                  >
                    <td className="px-4 py-3.5 text-sm text-slate-400">
                      {(state.page - 1) * state.pageSize + idx + 1}
                    </td>
                    {columns.map((col) => (
                      <td
                        key={col.key}
                    ...
                    <td className="px-4 py-3.5 text-right">
                      {state.deleteConfirmId === record.id ? (
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={() => setState((s) => ({ ...s, deleteConfirmId: null }))}
                            className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium hover:bg-slate-100 dark:border-slate-600 dark:hover:bg-slate-700"
                          >
                            Cancel
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="rounded-lg px-3 py-1.5 text-xs font-medium text-blue-600 transition-colors hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-950"
                          >
                            Edit
                          </button>
                          {(record as any).isDeleted ? (
                            <button
                              ...
                              className="rounded-lg px-3 py-1.5 text-xs font-medium text-emerald-600 transition-colors hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-950"
                            >
                              Restore
                            </button>
                          ) : (
                            <button
                              onClick={() => confirmDelete(record.id as string)}
        ...
        {state.totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-slate-100 px-4 py-3 dark:border-slate-700">
            <p className="text-sm text-slate-500">
              Showing {(state.page - 1) * state.pageSize + 1}–
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                onClick={() =>
                  setState((s) => ({ ...s, page: Math.min(s.totalPages, s.page + 1) }))
                }
