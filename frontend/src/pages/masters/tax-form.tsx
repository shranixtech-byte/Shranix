      ...
      apiRequest<TaxGroupForm>(`/tax-groups/${id}`)
        .then((data) => {
          if (data && typeof data === 'object') {
            setForm({ ...initialTaxGroup, ...data } as TaxGroupForm);
          }
        })
      ...
      fields: (
        <>
          ...
          <FormTextarea
            ...
            placeholder="Tax group description..."
          />
        </>
      ),
    },
  ...
  useEffect(() => {
    if (isEditing && id) {
      ...
      apiRequest<GstRateForm>(`/gst-rates/${id}`)
        .then((data) => {
          if (data && typeof data === 'object') {
            setForm({ ...initialGstRate, ...data } as GstRateForm);
          }
        })
        .catch((err) => setError((err as Error).message))
        .finally(() => setLoading(false));
      ...
      fields: (
        <>
          <FormInput
            label="IGST %"
            type="number"
            value={String(form.igst)}
            onChange={(e) => update('igst', Number(e.target.value))}
            placeholder="18"
          />
          <FormInput
            label="CGST %"
            type="number"
            value={String(form.cgst)}
            onChange={(e) => update('cgst', Number(e.target.value))}
            placeholder="9"
          />
          <FormInput
            label="SGST %"
            type="number"
            value={String(form.sgst)}
            onChange={(e) => update('sgst', Number(e.target.value))}
            placeholder="9"
          />
          <FormInput
            label="Cess %"
            type="number"
            value={String(form.cess)}
            onChange={(e) => update('cess', Number(e.target.value))}
            placeholder="0"
          />
        </>
      ),
