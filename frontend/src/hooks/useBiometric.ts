...
function detectBiometricType(): BiometricState['biometricType'] {
  if (typeof window === 'undefined') {return 'none';}

  const ua = navigator.userAgent.toLowerCase();

  if (ua.includes('face') || ua.includes('faceid')) {return 'face';}
  if (ua.includes('finger') || ua.includes('touch') || ua.includes('biometric')) {return 'fingerprint';}
  return 'none';
}

export function useBiometric(options?: BiometricOptions) {
  ...
  const authenticate = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const message = (error as Error).message || 'Biometric authentication failed';
      setState((prev) => ({
        ...prev,
        isAuthenticated: false,
        error: message,
...
export function useDeviceRegistration() {
  ...
  const removeTrust = useCallback(() => {
    if (!deviceId) {return;}
    const trustedDevices = JSON.parse(localStorage.getItem('shranix_trusted_devices') || '[]');
    const filtered = trustedDevices.filter((id: string) => id !== deviceId);
