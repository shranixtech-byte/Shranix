...
export const sqliteUsers = sqliteTableBase('shranix_users', {
  ...
  // JSON array string — e.g. '["sales","stock"]' — jinke modules dikhne hain
  allowedModules: sqliteText('allowed_modules'),
});
...
export const sqlitePermissions = sqliteTableBase(
  'shranix_permissions',
  {
    id: sqliteText('id')
      .primaryKey()
...
export const sqliteRolePermissions = sqliteTableBase(
  'shranix_role_permissions',
  {
    id: sqliteText('id')
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    roleId: sqliteText('role_id')
      .notNull()
      .references(() => sqliteRoles.id, { onDelete: 'cascade' }),
    permissionId: sqliteText('permission_id')
      .notNull()
      .references(() => sqlitePermissions.id, { onDelete: 'cascade' }),
  },
  (table) => ({
...
export const sqliteUserRoles = sqliteTableBase(
  'shranix_user_roles',
  {
    id: sqliteText('id')
      .primaryKey()
...
export const pgUsers = pgTableBase('shranix_users', {
  ...
  // JSON array string — jinke modules dikhne hain
  allowedModules: pgText('allowed_modules'),
});
...
export const pgPermissions = pgTableBase(
  'shranix_permissions',
  {
    id: pgUuid('id').primaryKey().defaultRandom(),
    createdAt: pgTimestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
...
export const pgRolePermissions = pgTableBase(
  'shranix_role_permissions',
  {
    id: pgUuid('id').primaryKey().defaultRandom(),
    roleId: pgUuid('role_id')
      .notNull()
      .references(() => pgRoles.id, { onDelete: 'cascade' }),
    permissionId: pgUuid('permission_id')
      .notNull()
      .references(() => pgPermissions.id, { onDelete: 'cascade' }),
  },
  (table) => ({
...
export const pgUserRoles = pgTableBase(
  'shranix_user_roles',
  {
    id: pgUuid('id').primaryKey().defaultRandom(),
