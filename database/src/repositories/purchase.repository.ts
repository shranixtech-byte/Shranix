...
export class PurchaseOrdersRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePurchaseOrders, pgPurchaseOrders, db, isPostgres); }
}
export class POItemsRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePOItems, pgPOItems, db, isPostgres); }
}
export class PurchaseQuotationsRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePurchaseQuotations, pgPurchaseQuotations, db, isPostgres); }
}
export class GrnRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqliteGrn, pgGrn, db, isPostgres); }
}
export class GRNItemsRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqliteGRNItems, pgGRNItems, db, isPostgres); }
}
export class PurchaseInvoicesRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePurchaseInvoices, pgPurchaseInvoices, db, isPostgres); }
}
export class PurchaseReturnsRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePurchaseReturns, pgPurchaseReturns, db, isPostgres); }
}
export class PurchaseReturnItemsRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePurchaseReturnItems, pgPurchaseReturnItems, db, isPostgres); }
}
export class SupplierPriceListRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqliteSupplierPriceList, pgSupplierPriceList, db, isPostgres); }
}
export class PurchaseApprovalsRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePurchaseApprovals, pgPurchaseApprovals, db, isPostgres); }
}
export class PurchaseSettingsRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePurchaseSettings, pgPurchaseSettings, db, isPostgres); }
}
export class SuppliersRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqliteSuppliers, pgSuppliers, db, isPostgres); }
}
export class PurchaseRequisitionsRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePurchaseRequisitions, pgPurchaseRequisitions, db, isPostgres); }
}
export class PurchaseRequisitionItemsRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqlitePurchaseRequisitionItems, pgPurchaseRequisitionItems, db, isPostgres); }
}
export class StockLedgerRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqliteStockLedger, pgStockLedger, db, isPostgres); }
}
export class WarehouseStockRepository extends MasterDataRepository<any> {
  constructor(db: DatabaseClient, isPostgres: boolean) { super(sqliteWarehouseStock, pgWarehouseStock, db, isPostgres); }
}
