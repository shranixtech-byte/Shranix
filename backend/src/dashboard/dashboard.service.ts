...
export class DashboardService {
  ...
  async getDashboard(userId: string) {
    ...
    const previousPurchases = this.sumWithin(purchaseInvoices, previousMonthStart, currentMonthStart);
    const lowStock = items.filter((item) => toNumber(item.currentStock) <= Math.max(toNumber(item.reorderLevel), toNumber(item.minStock)));
    const pendingApprovals = tasks.filter((task) => task.status === 'pending');
    const unreadNotifications = notifications.filter((notification) => notification.userId === userId && !notification.isRead);
    ...
    return {
      ...
      kpis: {
        ...
        inventoryValue: items.reduce((total, item) => total + toNumber(item.currentStock) * toNumber(item.purchaseRate), 0),
      ...
      purchases: {
        ...
        unpaid: purchaseInvoices.filter((invoice) => ['pending', 'unpaid', 'overdue'].includes(String(invoice.status))).length,
      },
      inventory: {
      ...
      insights: this.buildInsights({ lowStock, pendingApprovals, currentSales, previousSales, currentPurchases, previousPurchases }),
    };
  }

  private async list(repository: { findAll: (params: { page: number; pageSize: number }) => Promise<{ data: RecordData[] }> }): Promise<RecordData[]> {
  ...
  private sumWithin(records: RecordData[], start: Date, end: Date): number {
    return records.reduce((total, record) => {
      ...
      return date && date >= start && date < end ? total + toNumber(record.grandTotal || record.totalAmount) : total;
    }, 0);
  }
  ...
  private buildInsights(data: { lowStock: RecordData[]; pendingApprovals: RecordData[]; currentSales: number; previousSales: number; currentPurchases: number; previousPurchases: number }) {
    const insights: Array<{ type: 'positive' | 'warning' | 'info'; title: string; description: string; actionPath: string }> = [];
    if (data.lowStock.length) {insights.push({ type: 'warning', title: 'Replenishment required', description: `${data.lowStock.length} item(s) are at or below their reorder level.`, actionPath: '/inventory/items' });}
    if (data.pendingApprovals.length) {insights.push({ type: 'warning', title: 'Approvals waiting', description: `${data.pendingApprovals.length} workflow task(s) need attention.`, actionPath: '/workflow/approvals' });}
    if (data.previousSales > 0) {insights.push({ type: data.currentSales >= data.previousSales ? 'positive' : 'info', title: 'Sales trend', description: `Current-month sales are ${Math.abs(this.change(data.currentSales, data.previousSales) || 0)}% ${data.currentSales >= data.previousSales ? 'above' : 'below'} the prior month.`, actionPath: '/sales/dashboard' });}
    if (data.previousPurchases > 0) {insights.push({ type: 'info', title: 'Procurement trend', description: `Current-month purchases are ${Math.abs(this.change(data.currentPurchases, data.previousPurchases) || 0)}% ${data.currentPurchases >= data.previousPurchases ? 'above' : 'below'} the prior month.`, actionPath: '/purchase/dashboard' });}
    return insights;
  }
}
