...
export class SalesApprovalEngineService {
    ...
    const seedMatrices = [
      {
        ...
        approvers: JSON.stringify([
          { level: 1, role: 'manager', canOverride: false, minAmount: 0, maxAmount: 100000 },
          { level: 2, role: 'admin', canOverride: true, minAmount: 100000 },
          { level: 2, role: 'finance_head', canOverride: true },
        ]),
        isActive: true,
        createdAt: now,
        updatedAt: now,
      ...
      },
      {
        ...
        levelCount: 2,
        approvers: JSON.stringify([
          { level: 1, role: 'manager', canOverride: false, minAmount: 0, maxAmount: 50000 },
          { level: 2, role: 'admin', canOverride: true, minAmount: 50000 },
        ]),
        isActive: true,
        createdAt: now,
        updatedAt: now,
  ...
  private async sendEmail(_to: string, _subject: string, _body: string): Promise<void> {
    this.logger.log(`[EMAIL] To: ${_to}, Subject: ${_subject}`);
  }
  private async sendWhatsApp(_to: string, _message: string): Promise<void> {
    this.logger.log(`[WHATSAPP] To: ${_to}, Message: ${_message}`);
  }
  private async sendSms(_to: string, _message: string): Promise<void> {
    this.logger.log(`[SMS] To: ${_to}, Message: ${_message}`);
  }
  ...
  private async addNotification(
    ...
    await this.database.approvalNotifications
      .create({
        approvalId,
        recipientId,
        recipientRole,
  ...
  private async evaluateRules(params: {
    documentType: DocumentType;
    amount: number;
    discountPercent: number;
    gstAmount: number;
  }): Promise<{ requiresHigherLevel: boolean; requiredRole: string; requiredLevel: number }> {
    const rulesResult = await this.database.approvalRules.findAll({ page: 1, pageSize: 100 });
    const rules = (rulesResult?.data || []).filter(
      (r: any) => r.documentType === params.documentType && r.isActive !== false,
    );
    ...
    for (const rule of rules) {
      ...
      if (rule.field === 'amount') {
        val = params.amount;
      } else if (rule.field === 'discount_pct') {
        val = params.discountPercent;
      } else if (rule.field === 'gst_amount') {
        val = params.gstAmount;
      }
      ...
      switch (rule.operator) {
        ...
        case '==':
          ...
          break;
        case 'between':
          matched = val >= ruleVal && val <= Number(rule.value2 || ruleVal);
          break;
      }
  ...
  async submitForApproval(params: {
    ...
    // Check existing
    const existingResult = await this.database.salesApprovals.findAll({
      page: 1,
      pageSize: 10,
      search: params.documentId,
    });
    const existing = (existingResult?.data || []).find(
      (a: any) =>
        a.documentId === params.documentId && a.status !== 'cancelled' && a.status !== 'closed',
    );
    if (existing) {
      throw new BadRequestException(
        `Document ${params.documentNumber} is already in approval workflow (${existing.status})`,
      );
    }

    // Find matrix
    const matricesResult = await this.database.approvalMatrices.findAll({ page: 1, pageSize: 50 });
    const matrix = (matricesResult?.data || []).find(
      (m: any) => m.documentType === params.documentType && m.isActive !== false,
    );
    if (!matrix) {
      throw new BadRequestException(`No active approval matrix found for ${params.documentType}`);
    }

    const ruleResult = await this.evaluateRules(params);
    const totalLevels = ruleResult.requiresHigherLevel
      ? Math.max(matrix.levelCount || 1, ruleResult.requiredLevel)
      : matrix.levelCount || 1;
    const now = new Date().toISOString();
    const risk: RiskLevel =
      params.amount > 500000 ? 'high' : params.amount > 100000 ? 'medium' : 'low';
    const priority: Priority =
      params.priority || (risk === 'high' ? 'high' : risk === 'medium' ? 'medium' : 'low');
    ...
    const master = await this.database.salesApprovals.create({
      ...
      risk,
      creditStatus: 'normal',
      assignedTo: firstApprover?.userId || firstApprover?.role || '',
      assignedToName: firstApprover?.role || '',
      isOverdue: false,
      dueDate,
      createdAt: now,
      updatedAt: now,
    ...
    await this.database.approvalHistory.create({
      ...
      actionByName: params.createdByName,
      fromStatus: 'draft',
      toStatus: 'pending',
      level: 0,
  ...
  async assign(approvalId: string, userId: string, userName: string, dto: AssignDto): Promise<any> {
    ...
    await this.database.salesApprovals.update(approvalId, {
      assignedTo: dto.assignToUserId,
      assignedToName: dto.assignToUserName,
      updatedAt: new Date().toISOString(),
    });
    await this.database.approvalHistory.create({
      approvalId,
      ...
      actionByName: userName,
      fromStatus: master.status,
      toStatus: master.status,
      level: master.currentLevel || 1,
      comment: `Assigned to ${dto.assignToUserName}${dto.comment ? `: ${dto.comment}` : ''}`,
      timestamp: new Date().toISOString(),
    });
    await this.addNotification(
      ...
      'assigned',
      'assigned',
      `${master.documentNumber} has been assigned to you for review`,
    );
    this.sendEmail(
      dto.assignToUserName,
      `New Approval Assigned: ${master.documentNumber}`,
      `You have been assigned to review ${master.documentNumber}`,
    );
    if (this.audit) {
      await this.audit.log({
        userId,
  ...
  async bulkApprove(
    ...
    for (const id of approvalIds) {
      try {
        await this.approve(id, userId, userName, { comment });
        success++;
      } catch (e) {
        failed++;
  ...
  async bulkReject(
    ...
    for (const id of approvalIds) {
      try {
        await this.reject(id, userId, userName, { comment });
        success++;
      } catch (e) {
        failed++;
        errors.push(`[${id}] ${(e as Error).message}`);
  ...
  async bulkAssign(
    ...
    for (const id of approvalIds) {
      try {
        await this.assign(id, userId, userName, { assignToUserId, assignToUserName, comment });
        success++;
      } catch (e) {
        failed++;
        errors.push(`[${id}] ${(e as Error).message}`);
  ...
  async findAll(
    ...
    return this.database.salesApprovals.findAll({
      page: params.page || 1,
  ...
  async getHistory(approvalId: string): Promise<any[]> {
    const result = await this.database.approvalHistory.findAll({
      page: 1,
      pageSize: 200,
      search: approvalId,
    });
  ...
  async getComments(approvalId: string): Promise<any[]> {
    const result = await this.database.approvalComments.findAll({
      page: 1,
      pageSize: 100,
      search: approvalId,
    });
  ...
  async getNotifications(userId: string, unreadOnly = false): Promise<any[]> {
    const result = await this.database.approvalNotifications.findAll({
      page: 1,
      pageSize: 100,
      search: userId,
    });
    let data = (result?.data || []).filter(
      (n: any) => n.recipientId === userId || n.recipientRole === userId,
    );
    if (unreadOnly) {
  ...
  async getDashboardStats(): Promise<any> {
    ...
    return {
      ...
      approvedToday: data.filter((a: any) => a.status === 'approved' && a.updatedAt >= todayStart)
        .length,
      rejectedCount: data.filter((a: any) => a.status === 'rejected').length,
      totalCount: data.length,
      overdueCount: data.filter(
        (a: any) => a.status === 'pending' && a.dueDate && new Date(a.dueDate) < now,
      ).length,
  ...
  async updateMatrix(id: string, data: Partial<any>): Promise<any> {
    ...
    return this.database.approvalMatrices.update(id, {
      ...data,
      updatedAt: new Date().toISOString(),
    });
  }
  ...
  private validateAction(master: any, action: string): void {
    ...
    if (master.status === 'cancelled') {
      throw new BadRequestException('Document approval has been cancelled');
    }
    if (master.status === 'closed') {
      throw new BadRequestException('Document is closed');
    }
    if (master.status === 'draft' && action !== 'assign') {
      throw new BadRequestException('Document has not been submitted for approval');
    }
  }
  ...
  private calculateAverageApprovalTime(data: any[]): number {
    ...
    const total = approved.reduce(
      (s: number, a: any) =>
        s + (new Date(a.updatedAt).getTime() - new Date(a.createdAt).getTime()) / (1000 * 60 * 60),
      0,
    );
