...
import { ApiTags, ApiBearerAuth, ApiOperation, ApiParam, ApiResponse, ApiBody, ApiQuery } from '@nestjs/swagger';
...
export class PurchaseDashboardController {
  ...
  @ApiResponse({ status: 200, description: 'Dashboard data with KPIs, top suppliers, recent purchases' })
...
export class PurchaseDebitNoteController {
  ...
  @ApiOperation({ summary: 'Create debit note from purchase return (transactional — full rollback)' })
  @ApiParam({ name: 'returnId', description: 'Purchase return ID' })
  @ApiResponse({ status: 201, description: 'Debit note created and posted with full accounting reversal' })
  ...
  @ApiParam({ name: 'id', description: 'Debit note ID' })
  async cancel(@Param('id') id: string, @CurrentUser() u: { id: string }, @Body('reason') reason?: string) {
...
export class PurchasePostingController {
  ...
  @Permissions('purchase.create')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Post a purchase invoice with full accounting in single transaction (rollback on failure)' })
  @ApiParam({ name: 'invoiceId', description: 'Purchase invoice ID' })
  @ApiResponse({ status: 200, description: 'Invoice posted with journal entries, supplier ledger, GST ledger' })
  @ApiResponse({ status: 400, description: 'Validation failure — invoice already posted, cancelled, or supplier missing' })
