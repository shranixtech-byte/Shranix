...
export class PredictiveService {
  ...
  async getTrendAnalysis(metric: string, historicalData: number[]): Promise<{ trend: 'up' | 'down' | 'stable'; changePercent: number; description: string }> {
    if (historicalData.length < 2) {
      return { trend: 'stable', changePercent: 0, description: 'Insufficient data for trend analysis.' };
    }
    ...
    try {
      ...
      return { trend, changePercent, description: `${metric} has a ${trend}ward trend with ${Math.abs(changePercent).toFixed(1)}% change.` };
    }
  }

  private generateForecast(metric: string, currentValue: number, periods: number, _frequency: string): ForecastResult {
    ...
    for (let i = periods; i > 0; i--) {
      ...
      const actual = currentValue * (1 + (trend * (periods - i)) + (Math.random() - 0.5) * variability);
      dataPoints.push({
    ...
    const trendDir: 'up' | 'down' | 'stable' = changePercent > 3 ? 'up' : changePercent < -3 ? 'down' : 'stable';
