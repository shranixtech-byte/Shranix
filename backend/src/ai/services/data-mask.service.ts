...
const SENSITIVE_PATTERNS: Array<{ pattern: RegExp; replacement: string; name: string }> = [
  { pattern: /\b(?:password|passwd|pwd)\s*[:=]\s*['"]?[^'"\s]+['"]?/gi, replacement: 'password: ***MASKED***', name: 'password' },
  { pattern: /\b(?:secret|api[_-]?key|api_key)\s*[:=]\s*['"]?[^'"\s]+['"]?/gi, replacement: 'api_key: ***MASKED***', name: 'api_key' },
  { pattern: /\bJWT[_-]?SECRET\s*[:=]\s*['"]?[^'"\s]+['"]?/gi, replacement: 'JWT_SECRET: ***MASKED***', name: 'jwt_secret' },
  { pattern: /\b(?:token|access[_-]?token|refresh[_-]?token)\s*[:=]\s*['"]?[^'"\s]+['"]?/gi, replacement: 'token: ***MASKED***', name: 'token' },
  ...
  { pattern: /\b\d{2}[A-Z]{5}\d{4}[A-Z]{1}\d[Z]{1}[A-Z\d]{1}\b/g, replacement: '**XXXXX9999X*', name: 'gstin' },
...
export class DataMaskService {
  ...
  maskPrompt(messages: Array<{ role: string; content: string }>, role?: string): Array<{ role: string; content: string }> {
