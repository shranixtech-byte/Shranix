...
export class ReportEngine {
  ...
  async generateTrialBalance(params: {
    ...
    const accounts = await this.database.chartOfAccounts.findAll({ page: 1, pageSize: 5000 } as any);
    ...
    if (entries.data) {
      for (const entry of entries.data as any[]) {
        ...
        if (!accountMap[key]) {accountMap[key] = { debit: 0, credit: 0 };}

        // Apply filters
        if (params.fromDate && entry.entryDate < params.fromDate) {continue;}
        if (params.toDate && entry.entryDate > params.toDate) {continue;}
        if (params.branchId && entry.branchId !== params.branchId) {continue;}
        if (params.costCenterId && entry.costCenterId !== params.costCenterId) {continue;}
  ...
  async generateProfitLoss(params: {
    ...
    const incomeAccounts = trialBalance.accounts.filter((a: any) => a.accountType === 'income' || a.accountType === 'revenue');
    const expenseAccounts = trialBalance.accounts.filter((a: any) => a.accountType === 'expenses' || a.accountType === 'expense');
  ...
  async generateBalanceSheet(params: {
    ...
    const liabilityAccounts = trialBalance.accounts.filter((a: any) => a.accountType === 'liabilities');
    ...
    const totalLiabilities = liabilityAccounts.reduce((s: number, a: any) => s + Math.abs(a.closingBalance), 0);
    const totalEquity = equityAccounts.reduce((s: number, a: any) => s + Math.abs(a.closingBalance), 0) + (pnl.netProfit > 0 ? pnl.netProfit : 0);
    ...
    return {
      ...
      assets: {
        ...
        items: assetAccounts.map((a: any) => ({ name: a.accountName, code: a.accountCode, amount: a.closingBalance })),
      },
      liabilities: {
        total: Math.round(totalLiabilities * 100) / 100,
        items: liabilityAccounts.map((a: any) => ({ name: a.accountName, code: a.accountCode, amount: Math.abs(a.closingBalance) })),
      },
      equity: {
        total: Math.round(totalEquity * 100) / 100,
        items: [
          ...equityAccounts.map((a: any) => ({ name: a.accountName, code: a.accountCode, amount: Math.abs(a.closingBalance) })),
          ...(pnl.netProfit > 0 ? [{ name: 'Current Year Profit', code: 'PNL', amount: Math.round(pnl.netProfit * 100) / 100 }] : []),
          ...(pnl.netProfit < 0 ? [{ name: 'Current Year Loss', code: 'PNL', amount: Math.round(Math.abs(pnl.netProfit) * 100) / 100 }] : []),
        ],
      },
    };
  }
  ...
  async generateCashFlow(params: {
    ...
    if (entries.data) {
      for (const entry of entries.data as any[]) {
        if (!cashAccountIds.has(entry.accountId)) {continue;}
        if (params.fromDate && entry.entryDate < params.fromDate) {continue;}
        if (params.toDate && entry.entryDate > params.toDate) {continue;}
        ...
        if (['sales', 'purchase', 'expense', 'salary', 'receipt', 'payment'].includes(entry.voucherType)) {
          operatingActivities.push({ date: entry.entryDate, description: entry.narration, amount, voucherType: entry.voucherType });
          totalOperating += amount;
        } else if (['asset', 'fixed_asset', 'investment'].includes(entry.voucherType)) {
          investingActivities.push({ date: entry.entryDate, description: entry.narration, amount, voucherType: entry.voucherType });
          totalInvesting += amount;
        } else {
          financingActivities.push({ date: entry.entryDate, description: entry.narration, amount, voucherType: entry.voucherType });
    ...
    return {
      ...
      generatedAt: new Date().toISOString(),
      params,
      operatingActivities: { total: Math.round(totalOperating * 100) / 100, items: operatingActivities },
      investingActivities: { total: Math.round(totalInvesting * 100) / 100, items: investingActivities },
      financingActivities: { total: Math.round(totalFinancing * 100) / 100, items: financingActivities },
  ...
  async generateDayBook(params: { date: string; voucherType?: string; branchId?: string }) {
    ...
    const dayEntries = (entries.data || []).filter((entry: any) => {
      if (entry.entryDate?.split('T')[0] !== params.date) {return false;}
      if (params.voucherType && entry.voucherType !== params.voucherType) {return false;}
      return true;
    });
  ...
  async generateAccountStatement(params: {
    ...
    const filteredEntries = (entries.data || []).filter((entry: any) => {
      if (entry.accountId !== params.accountId) {return false;}
      if (params.fromDate && entry.entryDate < params.fromDate) {return false;}
      if (params.toDate && entry.entryDate > params.toDate) {return false;}
      return true;
    ...
    return {
      ...
      generatedAt: new Date().toISOString(),
      params,
      account: account ? { id: account.id, code: account.accountCode, name: account.accountName, type: account.accountType } : { id: params.accountId },
  ...
  async generateGeneralLedger(params: {
    ...
    const filteredEntries = (entries.data || []).filter((entry: any) => {
      if (params.fromDate && entry.entryDate < params.fromDate) {return false;}
      if (params.toDate && entry.entryDate > params.toDate) {return false;}
      if (params.accountId && entry.accountId !== params.accountId) {return false;}
      return true;
    });
  ...
  // ── GST REGISTER ───────────────────────────────────────
  async generateGstRegister(params: { fromDate?: string; toDate?: string; gstType?: string; gstin?: string }) {
    ...
    const filtered = (entries.data || []).filter((entry: any) => {
      if (params.fromDate && entry.voucherDate < params.fromDate) {return false;}
      if (params.toDate && entry.voucherDate > params.toDate) {return false;}
      if (params.gstType && entry.gstType !== params.gstType) {return false;}
      if (params.gstin && entry.gstin !== params.gstin) {return false;}
      return true;
    });
    ...
    const totalCess = filtered.filter((e: any) => e.gstType === 'CESS').reduce((s: number, e: any) => s + Number(e.gstAmount || 0), 0);
    ...
    return {
      ...
      params,
      entries: filtered,
      summary: { totalTaxable: Math.round(totalTaxable * 100) / 100, totalGst: Math.round(totalGst * 100) / 100, totalCess: Math.round(totalCess * 100) / 100 },
    };
  }

  // ── TAX LEDGER ─────────────────────────────────────────
  async generateTaxLedger(params: { fromDate?: string; toDate?: string; accountId?: string; financialYearId?: string }) {
  ...
  // ── AUDIT REPORT ───────────────────────────────────────
  async generateAuditReport(params: { fromDate?: string; toDate?: string; userId?: string; module?: string; action?: string }) {
    this.logger.log('Generating Audit Report');
    let logs: any[] = [];

    if (params.userId) {
      const result = await this.database.auditLogs.findByUserId(params.userId, { page: 1, pageSize: 10000 });
      logs = result.data || [];
    } else if (params.action) {
      const result = await this.database.auditLogs.findByEvent(params.action, { page: 1, pageSize: 10000 });
      logs = result.data || [];
    } else {
      // If no specific filter, use the most recent entries via findByEvent with empty event
      const result = await this.database.auditLogs.findByEvent('login', { page: 1, pageSize: 5000 });
      const result2 = await this.database.auditLogs.findByEvent('logout', { page: 1, pageSize: 5000 });
      logs = [...(result.data || []), ...(result2.data || [])];
    }

    const filtered = logs.filter((log: any) => {
      if (params.fromDate && log.createdAt < params.fromDate) {return false;}
      if (params.toDate && log.createdAt > params.toDate) {return false;}
      if (params.userId && log.userId !== params.userId) {return false;}
      if (params.module && log.resource !== params.module) {return false;}
  ...
  // ── GST SUMMARY ────────────────────────────────────────
  async generateGstSummary(params: { fromDate?: string; toDate?: string; gstin?: string; returnPeriod?: string }) {
    ...
    for (const entry of (entries.data || []) as any[]) {
      ...
      if (entry.inputOutput === 'input') {totalInputTax += Number(entry.gstAmount || 0);}
      else {totalOutputTax += Number(entry.gstAmount || 0);}
    }
  ...
  async generateYearClosingReport(params: { financialYearId: string; closingType?: string }) {
    ...
    const closingRecords = await this.database.yearClosingRecords.findAll({ page: 1, pageSize: 100 } as any);
    const closing = (closingRecords.data || []).find((r: any) => r.financialYearId === params.financialYearId);
  ...
  // ── FINANCIAL SUMMARY ─────────────────────────────────
  async generateFinancialSummary(params: { financialYearId?: string; periodKey?: string; branchId?: string }) {
    this.logger.log('Generating Financial Summary');
    const trialBalance = await this.generateTrialBalance({ financialYearId: params.financialYearId });
    const pnl = await this.generateProfitLoss({ financialYearId: params.financialYearId });

    const revenue = trialBalance.accounts.filter((a: any) => a.accountType === 'income' || a.accountType === 'revenue');
    const expenses = trialBalance.accounts.filter((a: any) => a.accountType === 'expenses' || a.accountType === 'expense');
    ...
    const accounts = await this.database.chartOfAccounts.findAll({ page: 1, pageSize: 5000 } as any);
    const cashAccounts = (accounts.data || []).filter((a: any) => a.isCashAccount);
    const cashBalance = cashAccounts.reduce((s: number, a: any) => s + Number(a.openingBalance || 0), 0);
